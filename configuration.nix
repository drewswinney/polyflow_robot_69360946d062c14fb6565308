{ config, pkgs, lib, webrtcPkg, webrtcEnv, pyEnv, robotConsoleStatic, robotApiPkg, rosWorkspace, ... }:

let
  user      = "admin";
  password  = "password";
  hostname  = "69360946d062c14fb6565308";
  homeDir   = "/home/${user}";
  ...
  pythonPath = lib.concatStringsSep ":" (lib.filter (p: p != "") [
    ...
    (lib.makeSearchPath "lib/python${py.pythonVersion}/site-packages" [ pyEnv rosWorkspace ])
  ]);
  ...
  workspaceLauncher = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      exec ${pkgs.python3}/bin/python3 "$WORKSPACE_LAUNCH"
    '';
...
in
{
  ...
  systemd.services.polyflow-rebuild = {
    ...
    serviceConfig = {
      ...
      StandardOutput = "journal";
      StandardError = "journal";
      ExecStart = "${polyflowRebuildRunner}/bin/polyflow-rebuild";
    };
  };
